      ...
      <a-form ref="valueRef" layout="horizontal" :hideRequiredMark="true" :rules="rules" :model="planBody" labelAlign="left">
        ...
        <a-form-item v-if="planBody.dock_sn" style="margin-top: -15px;">
          <div class="panel" style="padding-top: 5px;">
            ...
            <div class="ml10 mt5" style="color: hsla(0,0%,100%,0.65);">
              <span><RocketOutlined /></span>
              <span class="ml5">{{ dock.children?.nickname ?? '无飞行器' }}</span>
            </div>
          </div>
        </a-form-item>
        <!-- 任务类型 -->
        <a-form-item label="计划定时" class="plan-timer-form-item">
        ...
        <template v-if="planBody.task_type === TaskType.Condition">
          <!-- battery capacity -->
          <a-form-item label="当电池电量达到时启动任务" :labelCol="{span: 23}" name="min_battery_capacity">
          ...
          <!-- storage capacity -->
          <a-form-item label="当存储容量达到时启动任务（MB）" :labelCol="{span: 23}" name="storage_capacity">
...
const rules = {
  ...
  out_of_control_action: [{ required: true, message: '选择失控动作' }],
}
